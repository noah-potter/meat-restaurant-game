...
public class SeekBehaviour : SteeringBehaviour
{
  ...
  public override (float[] danger, float[] interest) GetSteering(float[] danger, float[] interest, AIData aiData)
  {
    ...
    targetPositionCached = aiData.currentTargetPosition;
    ...
    {
      float result = Vector2.Dot(directionToTarget.normalized, Directions.All[i]);
  ...
  private void OnDrawGizmos()
  {
    ...
    if (Application.isPlaying && interestsTemp != null)
    {
      if (interestsTemp != null)
      {
        ...
        {
          // Debug.LogFormat("SeekBehaviour: {0}", interestsTemp[i]);
          Gizmos.DrawRay(transform.position, Directions.All[i] * interestsTemp[i]);
        }
